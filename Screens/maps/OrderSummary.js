...
const OrderSummary = (props) => {
  ...
  const handleConfirm = async () => {
    const formData = {
      orderLevel: orderStatus,
    };

    axios
      .post(`${baseURL}order/${orderId}`, formData, {
        headers: {
      ...
      .then((res) => {
        if (res.status === 200 || res.status === 201) {
          ...
          // setTimeout(() => {
          //   navigation.navigate("OrderSummary", { orderId: orderId });
          // }, 500);
          //onRefresh();
        }
      })
  ...
  const radioButtons = useMemo(
    () => [
      {
        id: "Container has been picked up", // acts as primary key, should be unique and non-empty string
        label: "Container has been picked up",
        value: "Container has been picked up",
      },
      {
        id: "Delivered",
        label: "Delivered",
        ...
        <View style={styles.modalContainer}>
          <View style={styles.modalContent}>
            <View
              ...
              <Text style={{ fontSize: 20 }}>Select Order Status</Text>
              <TouchableOpacity onPress={toggleModal}>
                <Icon name="close" size={20} color="#13b7f2" />
              </TouchableOpacity>
            </View>
            <View style={styles.hr}></View>
            <RadioGroup
